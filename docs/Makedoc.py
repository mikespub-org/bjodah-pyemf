...
if __name__ == "__main__":
    ...
    parser.add_option("-m", action="store", dest="module", help="module to import")

    parser.add_option("-o", action="store", dest="outputfile", help="output filename")
    parser.add_option(
        "-n",
        "--name",
        action="append",
    ...
    parser.add_option(
        ...
        action="store_true",
        dest="printnamespace",
        help="print namespace and exit without processing",
    )
    parser.add_option(
        "-d",
        "--docstring-only",
        action="store_true",
        dest="docstringonly",
        help="only variable-expand the named file's docstring only; leave the remaining contents unchanged.",
    )
    ...
    if options.remapkey:
        for key1, key2 in options.remapkey:
            ...
            print(f"keyword={key1} value={value}")
            remap(key1, value)
